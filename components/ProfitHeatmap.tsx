...
const ProfitHeatmap: React.FC = () => {
  ...
  const zoneStatsMap = validJobs.reduce((acc: ZoneStatsMap, job: Job) => {
    ...
    if (job.zone) {
      zoneName = job.zone;
    } else if (job.address && job.address.includes(',')) {
    ...
    acc[zoneName].totalRevenue += (job.price_quote || 0);
    acc[zoneName].totalHours += ((job.duration_minutes || 0) / 60);
          ...
          <div key={zone.name} className="group">
            <div className="flex justify-between items-end mb-1">
              ...
              <div className={`font-bold text-sm ${getHeatText(zone.revenuePerHour)}`}>
                {settings.currency}{(zone.revenuePerHour || 0).toFixed(0)}<span className="text-xs text-slate-400 font-normal">/hr</span>
              </div>
            </div>
            ...
            <div className="flex justify-between mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <span className="text-[10px] text-slate-400">Avg Job: {settings.currency}{((zone.totalRevenue || 0) / (zone.jobCount || 1)).toFixed(0)}</span>
