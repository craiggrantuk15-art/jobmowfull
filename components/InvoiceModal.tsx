...
const InvoiceModal: React.FC<InvoiceModalProps> = ({ job, onClose }) => {
    ...
    const handleGenerateEmail = async () => {
        ...
        const type = job.payment_status === PaymentStatus.PAID ? 'receipt' : 'invoice';
    ...
    const handleSend = () => {
        ...
        addCommunication({
            ...
            subject: `${type} Sent`,
            body: emailDraft || `Sent ${type.toLowerCase()} for £${job.price_quote}.`,
            organization_id: '',
    ...
    const togglePayment = () => {
        const newStatus = job.payment_status === PaymentStatus.PAID ? PaymentStatus.UNPAID : PaymentStatus.PAID;
    ...
    const handlePaymentMethodChange = (method: string) => {
        updateJob(job.id, { payment_method: method });
    };
            ...
            <div className="bg-white rounded-xl max-w-2xl w-full shadow-2xl overflow-hidden relative animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                ...
                <div className="flex justify-between items-center p-4 border-b border-slate-100 bg-slate-50">
                    <h2 className="font-bold text-slate-800 flex items-center gap-2">
                        ...
                        {job.payment_status === PaymentStatus.PAID ? 'Payment Receipt' : 'Invoice'}
                    </h2>
                ...
                <div className="p-8 overflow-y-auto flex-1 bg-white">
                    ...
                    <div className="border border-slate-200 rounded-lg p-8 shadow-sm">
                        <div className="flex flex-col sm:flex-row sm:justify-between gap-4 items-start mb-10">
                            <div>
                            ...
                            <div className="text-right">
                                <h3 className="text-xl font-bold text-slate-900 mb-1">
                                    {job.payment_status === PaymentStatus.PAID ? 'RECEIPT' : 'INVOICE'}
                                </h3>
                                <p className="text-slate-500 text-sm">#{job.id.substring(0, 8)}</p>
                                <p className="text-slate-500 text-sm">{job.completed_date?.split('T')[0] || 'Unknown Date'}</p>
                            </div>
                        </div>
                            ...
                            <tbody>
                                <tr className="border-b border-slate-50">
                                    <td className="py-4 text-slate-800">
                                        {job.frequency} Lawn Mowing ({job.lawn_size})
                                        {job.notes && <div className="text-xs text-slate-500 mt-1">{job.notes}</div>}
                                    </td>
                                    <td className="py-4 text-slate-800 text-right font-medium">{settings.currency}{job.price_quote.toFixed(2)}</td>
                                </tr>
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td className="pt-4 text-right font-bold text-slate-900">Total</td>
                                    <td className="pt-4 text-right font-bold text-slate-900 text-xl">{settings.currency}{job.price_quote.toFixed(2)}</td>
                                </tr>
                            </tfoot>
                        </table>
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mt-8">
                            <div>
                                ...
                                <p className="text-sm text-slate-600 flex items-center gap-2">
                                    <CreditCard size={14} />
                                    {job.payment_method || 'Bank Transfer'}
                                </p>
                                {(job.payment_method || 'Bank Transfer') === 'Bank Transfer' && job.payment_status === PaymentStatus.UNPAID && (
                        ...
                        <div className="mt-6 bg-gradient-to-r from-amber-50 to-orange-50 border border-orange-200 rounded-lg p-4 animate-in zoom-in-95 duration-300">
                            ...
                            {!reviewDraft ? (
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-orange-700">Great job collecting payment! Want to ask {job.customer_name} for a Google Review?</p>
                                    <button
                ...
                {/* Footer Actions */}
                <div className="p-4 border-t border-slate-100 bg-slate-50 flex flex-col sm:flex-row flex-wrap gap-3 justify-between items-stretch sm:items-center">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2 mr-2">
                            ...
                            <select
                                ...
                                value={job.payment_method || 'Bank Transfer'}
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors flex items-center gap-2 ${job.payment_status === PaymentStatus.PAID
                                ? 'bg-slate-200 text-slate-700 hover:bg-slate-300'
                                : 'bg-emerald-600 text-white hover:bg-emerald-700'
                                }`}
                        >
                            {job.payment_status === PaymentStatus.PAID ? 'Mark Unpaid' : 'Mark as Paid'}
                        </button>
                        <button
                        ...
                        >
                            ...
                            {job.payment_status === PaymentStatus.PAID ? 'Send Receipt' : 'Send Invoice'}
                        </button>
                    </div>
