...
const Schedule: React.FC = () => {
  ...
  const handleOpenRainMode = async () => {
    ...
    const todayJobs = jobs.filter(j => 
        ...
        (j.scheduledDate?.split('T')[0] === today)
    );
  ...
  const confirmRainReschedule = () => {
    if (affectedJobs.length === 0) return;

    // Batch update jobs
    affectedJobs.forEach(job => {
        updateJob(job.id, { 
            scheduledDate: rainRescheduleDate,
            isRainDelayed: true,
             ...
             {pendingJobs.map(job => (
               ...
               >
                 <div className="flex justify-between items-start">
                   <span className="font-medium text-slate-800 text-sm">{job.customerName}</span>
                   <button 
                 ...
                 <div className="mt-2 flex gap-2">
                     <span className="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-500">{job.durationMinutes}m</span>
                 </div>
               </div>
             ))}
                ...
                <div key={job.id} className="relative pl-24 pb-8 group">
                    ...
                    >
                        <div className="absolute top-0 bottom-0 -left-[33px] w-0.5 bg-slate-200 group-last:bg-transparent">
                            <div className={`absolute top-4 -left-[5px] w-[11px] h-[11px] rounded-full border-2 border-white ring-1 ${job.isRainDelayed ? 'bg-blue-500 ring-blue-100' : 'bg-emerald-500 ring-emerald-100'}`}></div>
                        </div>
                        ...
                        <div className="flex justify-between items-start mb-1">
                            <div>
                                ...
                                <h4 className={`font-semibold ${job.isRainDelayed ? 'text-blue-900' : 'text-emerald-900'}`}>{job.customerName}</h4>
                            </div>
                            <div className="flex gap-2">
                                {job.phone && (
                                    <button 
                                        onClick={(e) => handleGenerateETA(e, job)}
                                        className={`${job.isRainDelayed ? 'text-blue-600 hover:text-blue-800 hover:bg-blue-200' : 'text-emerald-600 hover:text-emerald-800 hover:bg-emerald-200'} p-0.5 rounded transition-colors`}
                                        title="Draft ETA SMS"
                                    >
                                        {loadingEtaId === job.id ? <Loader2 size={16} className="animate-spin" /> : <MessageCircle size={16} />}
                                    </button>
                                )}
                                <span className={`text-xs font-bold ${job.isRainDelayed ? 'text-blue-700 bg-white/50' : 'text-emerald-700 bg-white/50'} px-2 py-0.5 rounded`}>
                                £{job.priceQuote}
                                </span>
                            </div>
                        </div>
                        
                        <div className={`flex items-center gap-4 text-sm ${job.isRainDelayed ? 'text-blue-800/80' : 'text-emerald-800/80'} mb-2`}>
                        ...
                        {job.notes && (
                            <div className={`text-xs p-2 rounded truncate ${job.isRainDelayed ? 'text-blue-700 bg-blue-100/50' : 'text-emerald-700 bg-emerald-100/50'}`}>
                            Note: {job.notes}
                            </div>
                        )}
                    </div>
                  ...
                  <div className="flex gap-3">
                      <button 
                          ...
                          className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                              copySuccess ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-white hover:bg-slate-700'
                          }`}
                      >
                      ...
                      <div className="p-6">
                          ...
                          <div className="mt-8 flex justify-end gap-3">
                              ...
                              <button 
                                ...
                                disabled={affectedJobs.length === 0 || !rainRescheduleDate}
                                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                  Confirm & Reschedule
                              </button>
                          </div>
