...
const Jobs: React.FC = () => {
    ...
    const [statusFilter, setStatusFilter] = useState<'ALL' | JobStatus.PENDING | JobStatus.SCHEDULED | JobStatus.COMPLETED | JobStatus.CANCELLED>('ALL');
    ...
    const handleCreateJob = (e: React.FormEvent) => {
        ...
        const job: Job = {
            ...
            lawn_size: newJob.lawnSize,
            price_quote: newJob.priceQuote,
            duration_minutes: newJob.durationMinutes,
            status: JobStatus.SCHEDULED,
    ...
    const renderCalendar = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const dayJobs = jobs.filter(j =>
                (j.scheduled_date?.split('T')[0] === dateStr ||
                    j.completed_date?.split('T')[0] === dateStr) &&
                j.status !== JobStatus.PENDING
            );

            const dayRevenue = dayJobs.reduce((acc, j) => acc + j.price_quote, 0);
                    ...
                    <div className="space-y-1">
                        {dayJobs.map(job => (
                            <div
                                ...
                                className={`px-2 py-1 rounded text-[10px] font-medium truncate cursor-pointer border transition-colors ${job.is_rain_delayed && job.status !== JobStatus.COMPLETED
                            ...
                            >
                                {job.is_rain_delayed && <CloudRain size={8} className="inline mr-1" />}
                                {job.customer_name}
                            </div>
                        ))}
                    </div>
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Jobs & Leads</h1>
                    <p className="text-slate-500">Manage bookings, scheduled work, and completed jobs.</p>
                </div>
                ...
                <>
                    <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-2 rounded-xl border border-slate-200 shadow-sm">
                        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
                            {['ALL', JobStatus.PENDING, JobStatus.SCHEDULED, JobStatus.COMPLETED, JobStatus.CANCELLED].map(status => (
                                <button
                                    key={status}
                                    onClick={() => setStatusFilter(status as any)}
                                    className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${statusFilter === status ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50'
                                        }`}
                                >
                                    {status === 'ALL' ? 'All' : status === JobStatus.PENDING ? 'New Leads' : status}
                                </button>
                            ))}
                        </div>
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between gap-4">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-2">
                                                <h3 className="text-lg font-bold text-slate-900 group-hover:text-lawn-700 transition-colors">{job.customer_name}</h3>
                                                <div className="flex items-center gap-2">
                                                    <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wide ${job.status === JobStatus.PENDING ? 'bg-blue-100 text-blue-700' :
                                                        job.status === JobStatus.SCHEDULED ? 'bg-emerald-100 text-emerald-700' :
                                                            job.status === JobStatus.COMPLETED ? 'bg-slate-100 text-slate-700' :
