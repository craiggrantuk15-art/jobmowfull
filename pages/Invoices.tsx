...
const Invoices: React.FC = () => {
    ...
    const filteredJobs = completedJobs
        .filter(j =>
            activeTab === 'unpaid'
                ? j.payment_status === PaymentStatus.UNPAID
    ...
    const totalOutstanding = completedJobs
        .filter(j => j.payment_status === PaymentStatus.UNPAID)
        .reduce((acc, j) => acc + j.price_quote, 0);

    const totalCollected = completedJobs
        .filter(j => j.payment_status === PaymentStatus.PAID)
        .reduce((acc, j) => acc + j.price_quote, 0);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl border border-red-100 shadow-sm flex items-center justify-between relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-sm text-slate-500">
                            Across <span className="font-semibold text-slate-700">{completedJobs.filter(j => j.payment_status === PaymentStatus.UNPAID).length}</span> unpaid invoices
                        </p>
                    </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-emerald-100 shadow-sm flex items-center justify-between relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-sm text-slate-500">
                            From <span className="font-semibold text-slate-700">{completedJobs.filter(j => j.payment_status === PaymentStatus.PAID).length}</span> paid jobs
                        </p>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-2 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex p-1 bg-slate-100 rounded-lg w-full sm:w-auto">
                    <button
                        ...
                        className={`flex-1 sm:flex-none px-6 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'unpaid' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                            }`}
                    >
                        Unpaid
                    </button>
                    <button
                        ...
                        className={`flex-1 sm:flex-none px-6 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'paid' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                            }`}
                    >
                        Paid History
                    </button>
                            ...
                            <div key={job.id} className="p-4 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1 text-xs text-slate-400">
                                            <Calendar size={12} />
                                            <span>Completed: {job.completed_date?.split('T')[0] || 'N/A'}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-0 border-slate-100">
                                    <div className="text-right mr-4">
                                        <p className="font-bold text-slate-900">£{job.price_quote.toFixed(2)}</p>
